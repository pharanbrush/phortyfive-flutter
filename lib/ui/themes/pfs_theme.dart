...
import 'package:pfs2/phlutter/widget/widget_state_property_utils.dart';
...
class PfsTheme {
  ...
  static ThemeData lysaris() {
    ...
    const Color buttonHoverBackgroundColor = Color.fromARGB(162, 125, 184, 197);
  ...
  static ThemeData phriends() {
    ...
    const Color popupBackground = Color(0xFF3C3D45);
    ...
    final newData = ThemeData(
      ...
      colorScheme: ColorScheme.fromSeed(
        ...
        outline: outline,
        surface: cardColor,
        surfaceContainer: popupBackground, // cards and popup colors
  ...
  static ThemeData clip() {
    ...
    const cspButtonRadius = 2.5;
    const buttonMaterialShape = RoundedRectangleBorder(
      borderRadius: BorderRadius.all(Radius.circular(cspButtonRadius)),
    );

    const popupBorder = BorderSide(color: Color(0xFF3F3F3F));
    const popupBorderShape = Border.fromBorderSide(popupBorder);
    ...
    final newData = ThemeData(
      ...
      colorScheme: ColorScheme.fromSeed(
        ...
        outline: outline,
        surface: cardColor,
        surfaceContainer: panelBackground, // cards and popup colors
...
class RectangleSliderThumbShape extends SliderComponentShape {
  ...
  Size getPreferredSize(bool isEnabled, bool isDiscrete) {
    return Size.fromRadius(isEnabled ? enabledThumbWidth : _disabledThumbWidth);
  }
  ...
  void paint(
    ...
    final rect =
        Rect.fromCenter(center: center, width: width, height: width * 0.5);
    final Path path = Path()..addRect(rect);

    bool paintShadows = true;
    assert(() {
      void debugDrawShadow(Canvas canvas, Path path, double elevation) {
        if (elevation > 0.0) {
          canvas.drawPath(
            path,
            Paint()
