...
final appRememberWindowSizeNotifier = ValueNotifier(false);
...
mixin WindowSizeRememberer<T extends StatefulWidget>
  ...
  void _handleWindowResize() {
    ...
    void queueSavetoPreferences() async {
      if (!appRememberWindowSizeNotifier.value) return;
      ...
      if (getLatestSaveCommandId() == saveCommandId) {
        if (appRememberWindowSizeNotifier.value) {
          storeCurrentWindowSize();
        }
      }
    }

    if (appRememberWindowSizeNotifier.value) queueSavetoPreferences();
  }
}
